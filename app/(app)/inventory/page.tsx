      ...
      <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_26px_60px_-28px_rgba(15,23,42,0.55)]">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <p className="font-semibold text-slate-900">Restock</p>
          </div>
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="rounded-xl border border-slate-100 p-3">
              ...
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Current</p>
            </div>
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
              <p className="text-2xl font-semibold text-emerald-700">{newTotal}</p>
              <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500">After Restock</p>
            </div>
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Units to add</label>
          ...
          <p className="text-xs text-slate-500">
            After restocking: <span className="font-semibold text-slate-700">{newDays >= 999 ? "∞" : newDays} days</span> of supply at {item.avgDailyPills}/day
          </p>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
          ...
          <button onClick={confirm} disabled={saving}
            ...
            {saving
              ...
              : <><Check className="h-3.5 w-3.5" />Confirm Restock</>}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative w-full space-y-5 pb-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">
            {criticalCount + lowCount > 0
              ? <span><span className="font-semibold text-red-600">{criticalCount + lowCount} medication{criticalCount + lowCount !== 1 ? "s" : ""}</span> need attention</span>
              : "All medications fully stocked"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Critical",  value: criticalCount, gradient: "from-red-500 to-rose-600",     accent: "text-red-700",    icon: ShieldAlert },
          { label: "Low Stock", value: lowCount,      gradient: "from-amber-400 to-orange-500", accent: "text-amber-700",  icon: TrendingDown },
          { label: "In Stock",  value: okCount,       gradient: "from-emerald-500 to-teal-600", accent: "text-emerald-700",icon: ShieldCheck },
        ].map(kpi => (
      ...
      {loading ? (
        ...
        <div className="dashboard-surface py-20 text-center">
          ...
          <p className="font-semibold text-slate-700">No inventory data</p>
          <p className="mt-1 text-xs text-slate-500">Add patients and medication schedules to track inventory</p>
        </div>
      ) : (
        <div className="dashboard-surface overflow-hidden">
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] items-center gap-4 border-b border-slate-100 bg-slate-50/80 px-6 py-3">
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400">Patient / Medication</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 text-right w-20">In Stock</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 text-right w-16">Avg/Day</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 text-right w-16">Days Left</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 text-center w-24">Status</p>
            <p className="w-24" />
          </div>
