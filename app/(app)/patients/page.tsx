    ...
    <div className="relative w-full space-y-5 pb-3">
      ...
      <div className="dashboard-surface flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">{patients.length} total patient{patients.length !== 1 ? "s" : ""} under active care</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="dashboard-surface py-20 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">Add your first patient to get started</p>
        </div>
      ) : (
                ...
                <div className="dashboard-surface group flex flex-col gap-4 p-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_20px_42px_-22px_rgba(15,23,42,0.45)] cursor-pointer">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-1">
                        {age !== null && (
                          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600">Age {age}</span>
                        )}
                        {patient.gender && (
                          <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-semibold text-blue-700">{patient.gender}</span>
                        )}
                      </div>
                      {patient.dob && <p className="mt-1 text-[11px] text-slate-400">DOB: {formatDate(patient.dob)}</p>}
                    </div>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-semibold text-slate-600">{medCount} medication{medCount !== 1 ? "s" : ""}</span>
                    </div>
